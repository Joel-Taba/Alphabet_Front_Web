...
//#region node_modules/.nitro/vite/services/ssr/assets/word-catalog-C-fKc0oN.js
function w(id, fr, en, es, theme) {
	return {
		id,
		fr,
		en,
...
new Map(WORD_CATALOG.map((word) => [word.id, word]));
/** Texte du mot dans la langue active. */
function wordText(word, lang) {
...
function lettersForWord(word, lang, style = "script") {
	return wordText(word, lang).split("").map((c) => getLetterFormation(c, style)).filter((l) => !!l);
}
var THEME_TITLES = {
	animaux: {
		fr: "Animaux",
...
var PALIER3_GROUP_MAP = new Map(PALIER3_GROUPS.map((g) => [g.id, g]));
/**
...
* (2 * indexDuNiveau + 2).
*/
function nextWordGroupAfterCrossword(level) {
	const levelIdx = PALIER3_CROSSWORD_LEVELS.indexOf(level);
	if (levelIdx < 0) return void 0;
	return PALIER3_GROUPS[2 * levelIdx + 2];
}
//#endregion
export { lettersForWord as a, WORD_CATALOG as i, PALIER3_GROUPS as n, nextWordGroupAfterCrossword as o, PALIER3_GROUP_MAP as r, wordText as s, PALIER3_CROSSWORD_LEVELS as t };
