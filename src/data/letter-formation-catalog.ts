/**
 * LETTER FORMATION CATALOG — Phase 2 : Combinatoire
 *
 * Chaque lettre est définie par une séquence ordonnée de signes de base,
 * chacun avec son propre chemin SVG positionné dans le contexte de la lettre
 * assemblée (viewport 200×200).
 *
 * Les chemins sont dérivés des signes individuels du sign-exercise-catalog.ts,
 * repositionnés pour former la lettre correcte selon la spécification
 * Flores Gong Nota.
 */

import type { SignFamily, LocalizedText } from "@/data/sign-exercise-catalog";

// ─── Types ────────────────────────────────────────────────────────────────────

/** Un signe individuel dans la séquence de formation d'une lettre */
export interface LetterSignStep {
  /** Famille du signe de base */
  family: SignFamily;
  /** Variante du signe */
  variant: string;
  /** Chemin SVG du signe, positionné dans le contexte de la lettre (viewport 200×200) */
  pathD: string;
  /** Point de départ du tracé de ce signe [x, y] */
  startXY: [number, number];
  /** Couleur du trait pour ce signe */
  strokeColor: string;
  /**
   * Ordre de superposition explicite (plus grand = dessiné au-dessus). Ne
   * concerne que les points d'intersection entre signes d'une même lettre —
   * quand absent, la priorité par défaut de la famille du signe s'applique
   * (voir `letterFamilyZIndex`). N'affecte jamais l'ordre de tracé/exercice.
   */
  zIndex?: number;
  /** Description courte du geste à effectuer */
  description: LocalizedText;
}

/** Définition complète de la formation d'une lettre */
export interface LetterFormation {
  /** Le caractère (ex: "a", "b", "1") */
  char: string;
  /** Nom lisible (ex: "a minuscule") */
  name: LocalizedText;
  /** Catégorie pédagogique */
  category: "voyelle" | "consonne" | "majuscule" | "chiffre";
  /** Zone verticale de la lettre sur la grille Seyès */
  zone: "corps" | "hampe" | "jambe";
  /** Séquence ordonnée des signes à tracer */
  steps: LetterSignStep[];
  /** Consigne vocale complète */
  consigne: LocalizedText;
}

// ─── Couleurs de référence par famille ────────────────────────────────────────
...
export const VOWELS: LetterFormation[] = [
  // ─── a ───
  // Formule : 1. courbe circulaire presque fermée (ouverte seulement à droite, horizontalement) + 2. trait vertical
...
export const UPPERCASE: LetterFormation[] = [
  ...
  },
 {
    ...
    steps: [
      { family: "trait", variant: "oblique-gauche", pathD: "M 60.06 15 L 78.53 125", startXY: [60.06, 15], strokeColor: C.trait, description: { fr: "Première oblique descendante", en: "First diagonal going down", es: "Primer oblicuo descendente", ar: "المائل الأول النازل" } },
      { family: "trait", variant: "oblique-droit", pathD: "M 97 15 L 78.53 125", startXY: [97, 15], strokeColor: C.trait, description: { fr: "Deuxième oblique montante", en: "Second diagonal going up", es: "Segundo oblicuo ascendente", ar: "المائل الثاني الصاعد" } },
      { family: "trait", variant: "oblique-gauche", pathD: "M 97 15 L 115.47 125", startXY: [97, 15], strokeColor: C.trait, description: { fr: "Troisième oblique descendante", en: "Third diagonal going down", es: "Tercer oblicuo descendente", ar: "المائل الثالث النازل" } },
      { family: "trait", variant: "oblique-droit", pathD: "M 133.94 15 L 115.47 125", startXY: [133.94, 15], strokeColor: C.trait, description: { fr: "Quatrième oblique montante", en: "Fourth diagonal going up", es: "Cuarto oblicuo ascendente", ar: "المائل الرابع الصاعد" } },
    ],
  },
   {
    char: "X",
...
export const LETTER_CATALOG: LetterFormation[] = [
  ...VOWELS,
  ...CONSONANTS,
  ...UPPERCASE,
  ...DIGITS,
...
export const LETTER_MAP: Map<string, LetterFormation> = new Map(
  LETTER_CATALOG.map((l) => [l.char, l])
);
