...
import { MobileShell, WordSearchPlay } from "@/components/amani";
...
/** "lvl2" → 2 mots dans la grille, "lvl3" → 3, etc. (même difficulté progressive que les mots croisés, Palier 3). */
...
function WordSearchScreen() {
  ...
  const wordSearch = useMemo(() => {
    ...
    return generateWordSearch(WORD_CATALOG, level, 1000 + level * 37);
  }, [level]);
  ...
  return (
    <MobileShell>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[13px] text-[#7A6A55] font-normal">
              {format(t.motsMeles.levelSubtitle, { level, count: wordSearch.placed.length })}
            </p>
          </div>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 bg-[#F5EDE0] pb-10 flex flex-col items-center">
        <WordSearchPlay wordSearch={wordSearch} puzzleId={puzzleId} level={level} />
      </div>
    </MobileShell>
  );
}
